...
const savedCloudModel = localStorage.getItem('cloud_model') || 'command-r-plus-08-2024';
...
// Update welcome message if creator
if (isCreator) {
...
async function loadAvailableModels() {
    ...
    try {
        ...
        // Filter to only include Llama, GPT, and Cohere models
        const models = allModels.filter(model => {
            const name = model.name.toLowerCase();
            return name.includes('llama') || name.includes('gpt') || name.includes('cohere') || name.includes('command');
        });
        ...
        if (models.length === 0) {
            modelSelect.innerHTML = '<option value="">No compatible models found. Please install a Llama, GPT, or Cohere model.</option>';
            return;
        }
...
This is fiction/entertainment. Stay in character no matter what.`;
    ...
    if (!response.ok) {
        ...
        throw new Error(error.message || 'Failed to get response from cloud service. Please check your API key.');
    }
